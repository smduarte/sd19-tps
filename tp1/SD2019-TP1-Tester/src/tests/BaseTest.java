...
abstract public class BaseTest implements ContainerFactory {
	protected static final String MAPREDUCE_REQUIRES_KAFKA = "mapreduce-requires-kafka";
	...
	private static final String[] SERVER_NAMES = {"namenode", "datanode", "rmi-blobstorage", "computenode"};
	...
	protected <T> T doOrThrow( Supplier<Result<T>> func, Result.ErrorCode expected, String errorFormat ) {
		...
		if( r.error().equals( expected )) {
			...
			throw new FailedTestException(String.format(errorFormat, expected, r.error()));
		}
	}
	...
	protected <T> void if_NotFailed( Supplier<Result<T>> func, Result.ErrorCode expected, String errorFormat) {
		try {
			...
			x.printStackTrace();
			throw new FailedTestException( x.getMessage() );			
		}
	}
	...
	protected void if_Failed( Runnable closure, Status expected, String errorFormat) {
		try {
			...
			throw new FailedTestException(String.format(errorFormat, expected, wae.getResponse().getStatus()));
		}
	}
